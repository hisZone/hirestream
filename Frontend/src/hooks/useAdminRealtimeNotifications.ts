        ...
        while (isActive) {
          ...
          for (const chunk of lines) {
            ...
            if (eventType === 'notification' && eventData) {
              try {
                const notification = JSON.parse(eventData) as AdminNotification

                // Immediately update unread count badge
                if (typeof notification.unread_count === 'number') {
                  queryClient.setQueryData(['admin-notifications-unread-count'], notification.unread_count)
                } else {
                  queryClient.setQueryData(['admin-notifications-unread-count'], (old: number | undefined) => (old ?? 0) + 1)
                }
                ...
                const message = notification.data?.message ?? 'A new notification requires your attention.'
